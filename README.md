...
The complete ordinary group-activity happy path—sign in, create a booking, list it under **My bookings**, cancel it, and observe its removal—has been manually verified against the real API on localhost. Public reads and the planned GitHub Pages origin's required CORS preflights were reverified without an API key on 2026-08-18; sanitized evidence is recorded in [`docs/public-runtime-evidence.md`](docs/public-runtime-evidence.md). The release-scope API integration is therefore considered technically verified. Waiting-list mutations, failure-specific responses, authenticated token renewal, and idempotency/rate limits remain unknown edge contracts rather than release blockers.
