...
- A sanitized `2026-08-07` CORS probe using `Origin: https://stranne.github.io`: the public schedule GET returned `Access-Control-Allow-Origin: *`; preflight requests for authenticated customer GET and POST operations allowed that origin, the `Authorization` and `Content-Type` headers, and GET/POST methods
...
| CORS, idempotency, and usage restrictions | Public schedule operations explicitly disable authentication. Authorized manual testing confirmed localhost browser CORS for login and the bearer-authenticated booking read/create/delete flow. A sanitized probe from the planned GitHub Pages origin confirmed permissive CORS headers for the public schedule GET and successful preflights for customer GET/POST requests carrying authorization and JSON content headers.                                                                | An authenticated end-to-end run from the deployed application, credentials policy, mutation idempotency key/header, retry policy, rate limits, acceptable-use/automation restrictions, and permission to ship this client remain unconfirmed. CORS is currently configured compatibly but is not a substitute for API usage permission and can change independently. |
...
2. Confirm API usage permission, rate limits, and mutation idempotency/retry rules; complete an authenticated end-to-end smoke test from the deployed origin even though its current CORS preflights succeed.
